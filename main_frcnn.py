...
def save(model):
    ...
    save_path = 'checkpoints_frcnn/fasterrcnn_%s' % timestr
...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(trainset, batch_size=1, shuffle=False, num_workers=2)

    testset = TestDataset(opt)
    test_loader = DataLoader(testset, batch_size=1, num_workers=2, shuffle=False, pin_memory=True)
    print('data loaded')

    model = FasterRCNN(n_fg_class=20, ratios=[0.5, 1, 2], scales=[8, 16, 32]).cuda()
